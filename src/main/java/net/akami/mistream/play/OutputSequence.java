 ...
 * when required (that is, when they reach the top of the queue and the current sequence gets terminated). <br>
 ...
 *         does NOT need to be determined before the alternative sequenced takes effect (that is, when they are required). <br>
 *         Since alternative sequences are not able to provide controllers themselves, they deliver the task to their chosen child
 *     </li>
 *     <li>
 *         {@link PolymorphicSequence}s are similar to {@link AlternativeSequence}s, but instead of defining which {@link OutputSequence}
 *         is suitable once for all (when the {@link #apply(LinkedList, DataHandler)} method is called for the first time), they constantly calculate which
 *         one is <b>currently</b> the most suitable, according to the respective weights of the handled children. <br>
 *         Beware that polymorphic sequences are not recommended for long sequences, since they might be costly performance-wise, as they
 ...
 *         be stopped immediately after they are applied, since they are not themselves able to provide a controller. Their
 *         only action is to put all their children to the top of the queue. They are extremely useful, because manually adding
 *         several sequences (instead of just one fragmented sequence) to the queue causes several issues :
 *         <ul>
 *             <li>
 *                 It becomes rather unclear what plays are currently present in the queue. Fragmented sequences are a very practical
 *                 way of grouping sequences, so that a name can be given for every play. In fact, "DiagonalKickoff" is much more
 *                 explicit than "[forward movement, right movement, side dash, ...]"
 *             </li>
 *             <li>
 *                 When the queue is too filled, it won't be updated anymore, since it is assumed that plays cannot be anticipated too much in advance.
 *                 A group of sequences counts as one "place" in the queue, therefore the next play can be predicted even if a 10 moves kickoff is prepared,
 *                 which is surely more logical, since that 10 sequences length play is nothing but a simple kickoff.
 *             </li>
 *             <li>
 ...
 *         describe bot actions that can not be fully determined beforehand. To illustrate that particular kind of sequence, {@link EndToEndSequence} would be
 *         a great example. Given a situation where the bot has to reach a certain destination, its steer and throttle can only be determined on
...
public interface OutputSequence {
    ...
    /**
     * Defines what actions are to be performed when the sequences is the current used one. Sequences <b>become</b> the "current used one"
     * whenever they meet the two following conditions : they are the head of the queue, and the previous current sequence is terminated
     * or null. <br>
    ...
    /**
     * Defines whether the sequence needs to be terminated. As soon as the {@code isStopped()} statement gets false,
     * the sequence will stop being applied.
     * @return whether the sequence should be terminated or not
     */
    boolean isStopped();

    /**
     * Allows plays to have concrete names, which might be useful for debugging purposes.
